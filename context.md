## Manual-Only Departures: Role-Locked Departed Status + Morning Pre-Departure Notification — 2026-07-18

Reworked the departure process per a three-part request: (1) disable automatic departures, (2) restrict the "mark Departed" action to Super Admin + Manager only, (3) email Super Admins/Managers each morning that guests are due to depart, so they can inspect/collect keys/checkout and then manually mark Departed.

**Investigation first — the "automatic departure" premise did not match the code.** Searched both repos exhaustively: no scheduler existed at all (`@nestjs/schedule` was not even installed; zero `@Cron`/`setInterval`/timeout jobs), and nothing derives `Departed`/`Dirty` from a clock/date comparison in any live path. A reservation becomes `Departed` **only** when a user explicitly sets that status; room→`Dirty` is set explicitly from the frontend on save ([ReservationsListPage.jsx](src/pages/ReservationsListPage.jsx) `markRoomDirtyIfNeeded`). The only date-derived `Departed` is dead mock code (`src/data/reservations.js` `getStatus`, hardcoded `today = 2025-12-31`) that no live page imports — both `/reservations` and `/reservations/list` read the real backend. Confirmed with the user, who chose to build the permission lock + notification and treat "disable auto-departure" as already satisfied (the live flow was already manual). Nothing was removed.

### 1. "Departed" locked to Super Admin (admin) + Manager — backend enforced
The UI's departure action is the **"Checked Out"** status option, which the backend normalizes to `BookingStatus.DEPARTED` (`normalizeBookingStatusInput`). New shared `Rms Booking Backend/src/helpers/departure-permission.helper.ts` → `ensureCanMarkDeparted(role, incomingStatus, existingStatus?)`: throws `ForbiddenException` only on the **transition into** Departed (incoming normalizes to DEPARTED **and** existing does not) by a role outside `[admin, manager]`. Transition-based (not blanket) so unrelated edits to an already-departed record by a `user` are not blocked, and create paths (no existing) still guard correctly. Wired into `reservations.service.ts` (`create` + `update`) and `booking.service.ts` (`update` + `updateStatus`); booking `create` needs no guard — it derives status from the already-guarded reservation. Single source of truth shared by both services so they can't drift.

*Frontend (UX only, backend is the real gate):* new reusable helpers in `src/data/options.js` — `canRoleMarkDeparted(role)`, `getSettableStatusOptions(role)`, `getStatusOptionsForDeparturePermission(canMarkDeparted)`. The **"Checked Out"** option is hidden from the status **setter** for non-admin/manager in the two setter locations: `ReservationsListPage.jsx` (`FormField` Status, via `getSettableStatusOptions(currentUser?.role)`) and `ReservationFormDrawer.jsx` (new `canMarkDeparted` prop, passed from `ReservationsPage.jsx` via `canRoleMarkDeparted(currentUser?.role)`; `PortalReservationsPage.jsx` keeps the default `false`). Status **filters** deliberately left untouched everywhere (`ReservationFilters.jsx`, `BookingChartHeader.jsx`, `PortalReservationsPage.jsx`'s "Filter by status") — any role may still filter by Checked Out to view already-departed guests.

### 2. Morning-of-departure-day email digest (new scheduler)
Installed `@nestjs/schedule` (v6.1.3, compatible with Nest 11) + `ScheduleModule.forRoot()` in `app.module.ts`. New `Rms Booking Backend/src/modules/reservations/departure-notifications.service.ts` (`DepartureNotificationsService`, provider in `ReservationsModule`): `@Cron('0 7 * * *')` daily at 07:00 server time. Finds reservations with `checkOut` = today whose status is not already Departed/Canceled (`expandStatusVariants` for both spellings) and not yet notified today, resolves recipients via new `UserService.findAdminAndManagerRecipients()` (all admin+manager users with an email, **not** client-scoped), and sends one **digest** email via new `NotificationsService.sendDepartureDueEmails()` (plain-text + escaped HTML table of guest/room/reservation/depart-time). Idempotent: new `Reservation.departureNoticeSentOn` (YYYY-MM-DD) is stamped after sending so a same-day restart/re-run can't resend. Cron logic is extracted into `notifyDueDepartures(referenceDate?)` for direct/manual invocation.

**Verified:** backend `tsc --noEmit` + `nest build` clean; app boots successfully (`ScheduleModule` + `ReservationsModule` with the new provider resolve with no DI errors, "Nest application successfully started"). Guard logic proven against the compiled `dist` helper: `user`/`portal_user`/`housekeeper` attempting to set Checked Out/Departed → `ForbiddenException`; `admin`/`manager` → allowed; `user` re-saving an already-Departed record or making non-departure changes → allowed. Frontend `npm run build` clean. The 07:00 cron was **not** fired manually and no test emails were sent — the `.env` points at a live Atlas DB + real Gmail SMTP, so live end-to-end email verification is deferred to a controlled run (invoke `notifyDueDepartures()` against seed data, or wait for 07:00) rather than emailing real admins/managers.

---

## Client Booking-Chart Ownership, OOS/OOO Client Booking Block, Booking Type Dropdown, Invoice Edit-Request Workflow, Booking Requests Removal — 2026-07-05

Six unrelated items reported together. Researched via two parallel Explore agents (frontend/backend) plus direct live verification (browser automation + API calls against the running dev servers) before any code was written; a Plan agent stress-tested the design before implementation; one design ambiguity (invoice tenant isolation, see #5) was confirmed directly with the user rather than assumed — his answer reversed my initial (incorrect) reading.

### 1. Invoice date pickers Monday-first — already fixed, no code change
Live-tested every `DatePicker` on `InvoiceGeneratorPage.jsx`/`InvoiceLineItemsTable.jsx` against the current dev server (git HEAD `fbb52b4`, same commit that added the 2026-07-04 global `dayjs.updateLocale` fix below) — all already render `Mo Tu We Th Fr Sa Su`. Confirmed via grep that no file calls `dayjs.locale(...)`/`dayjs.updateLocale(...)` to re-register a conflicting locale. The reported screenshot was from a stale build; nothing to fix.

### 2. Client's own reservations no longer masked as "Reserved by Another Guest"
`src/components/BookingChart.jsx` previously showed `Reserved By: {company}` (bar label, reduced hover-popover fields, popover title) for **every** booking when `isPortalUser`, with zero ownership check — a client's own reservation looked identical to another client's. Fixed by comparing each booking's `billingClientNumber` (already present in the `/bookings/chart` payload, confirmed live) against the logged-in user's own `useAuthStore(s => s.user?.clientNumber)` (equals `linkedClientNo` for portal accounts) via a new `isOwnedBooking(booking)` helper. Own bookings (and anything admin-assigned to this client, since ownership keys off the *billed-to* tenant, not `createdBy`) now render exactly like admin sees them; only genuinely-other-client bookings get the masked view.

**Real gap found and fixed, not just the reported symptom:** the resize handles and the right-click "Edit"/"Open in Reservations"/"Assign Housekeeping Task" menu items were previously rendered **unconditionally** for portal users — a client could attempt to resize or open another client's booking in the UI (the backend's `ensureBookingScope()` would 403 it, but the UI offered the action at all). Both are now gated behind the same `isOwnedBooking` check; a non-owned booking's context menu now shows only a read-only "Reserved By: X" header + "Close Menu". Drag-start was already fully `isPortalUser`-blocked (admin-only feature, unrelated to ownership) — untouched.

**Verified:** backend confirms `billingClientNumber` flows through correctly; frontend build clean. Full browser-rendered confirmation (drag/resize/context-menu interaction as a live portal user) blocked by this environment's mandatory-2FA-on-first-login wall for non-admin roles — flagged to the user rather than bypassed.

### 3. Client bookings blocked on Out-of-Service/Out-of-Order rooms (Admin/Manager unaffected)
`booking.service.ts`'s `checkAvailability()` only ever checked overlapping `Booking` documents, never `Room.serviceEntries` (the OOS/OOO array) — any role could book straight over a room marked unavailable. New `ensureRoomBookableForRequestUser(roomId, start, end, requestUser?)` on `BookingService` — no-ops unless `requestUser.role === Role.PORTAL_USER`, otherwise checks for a date-overlapping `serviceEntries` entry and throws a `ConflictException` with a clear message. Called immediately before the existing `checkAvailability` check at all 3 real call sites: `booking.service.ts`'s `create()`/`update()`, and `reservations.service.ts`'s private `ensureRoomAvailability()` (which needed a new `requestUser` param threaded through — portal users only ever reach reservation creation via `/reservations`, which delegates into this same method, so this one change covers every path).

**Verified live end-to-end**: created a disposable portal account + temporary future OOS entry on a real room — portal booking attempt on that room/dates → 409 with the new message; portal booking a *different* room same dates → 201 success; Admin booking the *same OOS room/dates* → 201 success (unaffected). Test reservation and OOS entry removed after.

### 4. Booking Type dropdown re-enabled for Client
`ReservationsListPage.jsx`'s `bkgSource` field had an explicit `disabled={isPortalUser}` from an earlier "view-only for Client" decision — user explicitly reversed this ("fix the dropdown so it opens... allows the user to select an option"). Removed the `disabled` prop and stale comment; backend already accepted `bkgSource` writes from `portal_user`. Deliberately **not** extended to `ReservationFormDrawer.jsx`'s separate `bookingSource` field (a plain disabled `<Input>`, not a dropdown, used only by the simpler `PortalReservationsPage.jsx` "My Reservations" drawer, admin sees the same plain input there too) — different UI element than the one reported.

### 5. Manager invoice history + edit-request-to-admin workflow
**Investigated before assuming a fix was needed**: `invoices.service.ts`'s `findAll()`/`ensureScope()` were already correctly tenant-scoped (Manager sees only invoices under their own `clientNumber`) — confirmed live that the reported "empty Invoice History for Manager Ahsan" was because **zero** invoices existed under `CL-998`, not a scoping bug. Asked the user directly whether "view all saved invoices" meant fully cross-tenant (like Admin) or view-all-of-their-own; user confirmed strict per-manager isolation must stay exactly as-is — no change made to `findAll()`/`ensureScope()`. The actual gap: no way for a Manager to regain edit access once their own invoice's existing 5-day window (`InvoicesService.canEdit()`) lapses.

Built a new module, `src/modules/invoice-edit-requests/` (backend), mirroring the existing `two-factor-requests` module's schema/service/controller/module shape exactly (including its `UserModule` import to resolve `fullName`, since the JWT has none): `InvoiceEditRequest` schema (invoice ref, invoiceNo, requestedBy/username/fullName/clientNumber, status enum Pending/Approved/Rejected, adminNotes, resolvedBy/At). `create()` enforces the *same* tenant isolation as invoices itself (`invoice.clientNumber === requestUser.clientNumber`, Forbidden otherwise) plus rejects requests for invoices still inside the window and duplicate Pending requests. `findAll()` scopes non-admin to their own `requestedBy`. `approve()`/`reject()` are Admin-only and just flip status — the actual unlock happens dynamically in `InvoicesService.canEdit()`, extended to check for an Approved request (by this specific manager, for this specific invoice) once the 5-day window has passed. Approval is indefinite/non-consumed (no expiry mentioned in the request).

**Real bug caught during implementation, not before:** initially duplicated the `EDIT_WINDOW_DAYS`/`MS_PER_DAY` constants as separate local consts in both `invoices.service.ts` and the new `invoice-edit-requests.service.ts` — a future change to one without the other would have let a Manager successfully request edit access for an invoice that's already outside one service's window but still "inside" the other's stale copy. Fixed by extracting both into a shared `src/constants/invoice.constants.ts`, imported (aliased) by both services.

Frontend: `src/api/services/invoiceEditRequests.js` + `src/hooks/useInvoiceEditRequests.js` (mirrors the `twoFactorRequests` pair). `InvoiceHistoryTable.jsx`/`InvoiceHistoryPage.jsx` — Manager rows where `canEdit === false` get a "Request Edit Access" button (becomes a disabled "Pending Approval" state once a Pending request exists for that invoice, tracked via the Manager's own `useInvoiceEditRequests()` list). New `src/pages/InvoiceEditRequestsAdminPage.jsx` (mirrors `TwoFactorRequestsAdminPage.jsx`), route + nav item (`/invoice-edit-requests`, admin-only), added to `ProtectedRoute.jsx`'s `ADMIN_PATHS`.

**Verified live, full lifecycle**, using disposable Manager accounts (2 different tenants) created and deleted via the API — no real accounts touched: Manager creates invoice → `canEdit: true` → (temporarily set the shared constant to 0 days to simulate an expired window, mirroring this project's own established precedent for testing time-boxed windows without waiting real days, then reverted) → `canEdit: false`, PATCH → 403 with a message directing to request access → Manager submits edit request → Pending created, a duplicate request correctly rejected → **a second Manager (different tenant) confirmed unable to see the invoice at all, unable to request edit on it (403 "your own invoices" only), and blocked by role-guard from approving anything** → Admin sees the Pending request, approves it → Manager's `canEdit` flips to `true`, PATCH now succeeds. All disposable clients/users/invoice deleted afterward. One harmless known leftover: the test `InvoiceEditRequest` audit row has no delete endpoint (matches the existing `two-factor-requests` precedent of never adding one) and remains visible on the real `/invoice-edit-requests` admin page.

### 6. "Booking Requests" removed for Admin/Manager (commented out, not deleted)
No literal dashboard widget existed for this (`DashboardPage.jsx` has zero references) — the whole admin/manager-facing nav entry + route was the actual target. Commented out (per explicit "just comment it out for now" instruction, so it's trivial to re-enable): the nav item in `navigation.js` (`roles: ['admin','manager']`), the lazy import + `<Route path="/booking-requests">` in `App.jsx`, and the `/booking-requests` entry in `ProtectedRoute.jsx`'s `ADMIN_PATHS`. Deliberately untouched: Client's own `/portal/booking-requests` route and `PortalDashboardPage.jsx`'s "Booking Requests" stat-card section — the request explicitly scoped this to "the Admin and Manager sides" only.

**Verified live**: nav item gone for admin, `/invoice-edit-requests` nav item (new, from #5 above) present; Invoice History and Invoice Edit Requests admin pages both render cleanly with correct data (confirmed via live DOM snapshot, zero new console errors beyond pre-existing antd deprecation warnings unrelated to this session's changes).
